...
public class Exercises {
	...
	public boolean doubleX(String str) {
		...
		if (x == -1) {
			return false;
		} else if (str.substring(x).startsWith("xx")) {
			return true;
		} return false;
	}
	...
	public int last2(String str) {
		...
		String endTwo = str.substring(str.length()-2, str.length());
		int counter = 0;
		for (int i = 0; i < str.length()-1; i++) {
			if (str.substring(i).startsWith(endTwo)) {
				counter++;
			} counter += 0;
		} return counter - 1;
	}
		...
		if (str.length() < 2) {
			...
			for (int i = 1; i < str.length()-1; i++) {
				if (str.substring(i, i+1).equals("x")) {
					...
					minusX += str.substring(i, i + 1);
				}
			}
		} return str.substring(0, 1) + minusX + str.substring(str.length()-1);
	}
