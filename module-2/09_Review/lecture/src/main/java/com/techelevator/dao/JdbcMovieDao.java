...
public class JdbcMovieDao implements MovieDao{
    ...
    public Movie create(Movie movie) {
        String sql = "INSERT INTO movie (title, overview, tagline, poster_path, home_page, " +
                "release_date, length_minutes, director_id, collection_id " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING movie_id;";

        Long newId = jdbcTemplate.queryForObject(sql, Long.class,
        movie.getTitle(), movie.getOverview(), movie.getTagline(), movie.getPosterPath(), movie.getHomePage(),
        movie.getReleaseDate(), movie.getLengthInMinutes(), movie.getDirectorId(), movie.getCollectionId());

        Movie newMovie = getMovie(newId);
    ...
    public void update(Movie movie) {
        ...
        jdbcTemplate.update(sql, movie.getTitle(), movie.getOverview(), movie.getTagline(), movie.getPosterPath(), movie.getHomePage(),
                movie.getReleaseDate(), movie.getLengthInMinutes(), movie.getDirectorId(), movie.getCollectionId(), movie.getMovieId());
    }
    ...
    public List<Movie> searchByTimeRange(int minimumLength, int maximumLength) {
        ...
        String sql = "SELECT movie_id, title, overview, tagline, poster_path, home_page, " +
                "release_date, length_minutes, director_id, collection_id" +
                "FROM movie " +
                "WHERE length_minutes BETWEEN ? AND ?;";
